    ...
    <div class="sl-pagebody">
        <div class="card pd-20 pd-sm-40">
            <div class="sl-page-title d-flex justify-content-between">
                <div>
                    <h4 class="card-header-title">Post Category</h4>
                    <p class="mg-b-20 mg-sm-b-30 tx-gray-600">This is a list of all post categories. You can add, edit, delete, or update the status of categories from here. You can also search, sort, and filter the list of categories as needed.</p>
                </div>
                <div>
                    <a class="btn" href="{{ route('post-category.create') }}"><i class="bi bi-plus-circle tx-24"></i></a>
                </div>
            </div>
...
<script>
    ...
    function statusChange(id) {
        var url = "{{ route('post-category.status',':id') }}";
        $.ajax({
            type: "GET",
    ...
    function deleteItem(id) {
        var url = "{{ route('post-category.destroy',':id') }}";
        $.ajax({
            type: "DELETE",
            url: url.replace(':id', id),
            success: function(resp) {
